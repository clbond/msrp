   ...
   public boost::enable_shared_from_this<Connection>
{
   public:
      ...
      boost::signal1<void, const asio::error_code&>& onDisconnect();
   ...
   private:
      ...
      boost::signal1<void, const asio::error_code&> mDisconnect;
      ...
      void connect();
      void connectHandler(const asio::error_code&);

      void reconnect(const asio::deadline_timer::duration_type&);
      void reconnectHandler(const asio::error_code&);

      void receiveHandler(const asio::error_code&, std::size_t bytes);

      void process();

      void write();
      void writeHandler(bool buffered, const asio::error_code&, std::size_t bytes);

      void listen(const asio::ip::tcp::endpoint&);
      void acceptHandler(const asio::error_code&);

      void disconnect(const asio::error_code&);
