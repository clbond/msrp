   ...
   for (; i != mOutgoing.end(); ++i)
   {
      if (i->get()->messageId() == id)
      {
         ...
         if (c)
         {
            ...
            // report if so.
            //
            // Actually upon reexamination, can't we just store this always?
            // In a client situation that is fine -- we will always know where
            // to send reports.  But in a proxy situation it's a serious memory
            // leak (and even in extreme client cases).  Well it's a memory leak
            // in any case, the severity depending on the stack load.
